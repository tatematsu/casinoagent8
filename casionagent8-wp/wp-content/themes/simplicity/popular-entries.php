...
<li class="popular-entry">
  ...
  <?php if ( has_post_thumbnail() ): // サムネイルを持っているときの処理 ?>
    <a href="<?php the_permalink(); ?>" class="popular-entry-link" title="<?php the_title(); ?>"><?php the_post_thumbnail( 'thumb100', array('style' => 'width:75px;height:75px;', 'alt' => get_the_title()) ); ?></a>
  <?php else: // サムネイルを持っていないときの処理 ?>
    <a href="<?php the_permalink(); ?>" class="popular-entry-image" title="<?php the_title(); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/no-image.png" alt="NO IMAGE" style="width:75px;height:75px;" class="no-image popular-list-no-image" /></a>
  <?php endif; ?>
